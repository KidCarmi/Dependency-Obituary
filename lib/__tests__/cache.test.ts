...
describe("buildCacheKey", () => {
  it("builds correct key for npm package", () => {
    expect(buildCacheKey("npm", "react", "18.2.0")).toBe("v8:dep:npm:react:18");
  });

  it("builds correct key for pypi package", () => {
    expect(buildCacheKey("pypi", "requests", "2.28.0")).toBe(
      "v8:dep:pypi:requests:2"
    );
  });

  it("handles version with no dots", () => {
    expect(buildCacheKey("npm", "left-pad", "1")).toBe("v8:dep:npm:left-pad:1");
  });

  it("handles empty version string", () => {
    expect(buildCacheKey("npm", "foo", "")).toBe("v8:dep:npm:foo:0");
  });
});
